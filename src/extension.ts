...
import { RouteTreeProvider, RouteNode, RouteTreeItem } from './routeTree';
...
export function activate(context: vscode.ExtensionContext) {
    ...
    (item?: RouteTreeItem | RouteNode) => {
      ...
      if (!route) {
        vscode.window.showInformationMessage(
          'Select a route in the Express Routes view first.'
        );
        return;
      }
  ...
  // Watch JS/TS files and auto-refresh routes when they change
  if (workspaceFolder) {
...
/**
 * Normalize the argument we get from VS Code:
 * - If called from the tree item's built-in command, we may get a RouteNode.
 * - If called from the context menu, we get a RouteTreeItem.
 * - If called from the command palette with no arg, we use the current selection.
 */
...
/**
 * Opens a Postman-like webview to test the route.
 */
...
function getTesterHtml(method: string, url: string): string {
